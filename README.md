...
| **Ownership proofs (SLIP-19)** | ✅ device-native | ✅ segwit + taproot — **verified on a real Mk4** (147-byte ECDSA / 105-byte BIP-340), both accepted by Wasabi's own `OwnershipProof.VerifyOwnership`; taproot output key derived through NBitcoin's BIP-86 tweak as an independent check | ✅ segwit + taproot (spec vector + BIP-86 vector) | ✅ on-device `create_proof` P2WPKH + P2TR (SLIP-21 ownership key); vectors pass Wasabi's verifier | ❌ not implemented (`SIGN_MESSAGE` can't produce SLIP-19 sighash format) | ❌ not implemented |
