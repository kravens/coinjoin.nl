...
|---|---|---|---|---|---|---|---|
| **Wasabi branch** | [`feature/trezor-coinjoin`](https://github.com/kravens/WalletWasabi/tree/feature/trezor-coinjoin) | [`feature/coldcard-coinjoin`](https://github.com/kravens/WalletWasabi/tree/feature/coldcard-coinjoin) | [`feature/passport-coinjoin`](https://github.com/kravens/WalletWasabi/tree/feature/passport-coinjoin) | [`feature/krux-coinjoin`](https://github.com/kravens/WalletWasabi/tree/feature/krux-coinjoin) | none — feasibility study only | none needed — device side in [`SabiSigner`](https://github.com/kravens/SabiSigner/tree/feat/ownership-proof) (app, branch `feat/ownership-proof`) + [`SabiSigner-os`](https://github.com/kravens/SabiSigner-os/tree/sabisigner) (image); host side is [`kruxd.py sabi`](kruxd/README.md#sabisigner-target) | none — feasibility study only |
| **Wasabi client side** | ✅ TrezorKeyChain + bridge | ✅ ColdcardKeyChain + raw USB HID (no bridge daemon) — hardware-verified on a real Mk4 | ✅ PassportKeyChain (transport moving USB HID → QuantumLink) | ✅ `KruxKeyChain` + [kruxd](kruxd/kruxd.py) bridge on :21326 (serial COM8 / simulator TCP); `importkruxwallet` RPC, per-wallet round/fee policy | ❌ none (USB channel + [client libs](https://github.com/LedgerHQ/app-bitcoin) exist, no KeyChain) | ✅ via bridge, no C# — [`kruxd.py sabi`](kruxd/README.md#sabisigner-target) speaks the SabiSigner encrypted HID session and presents the Krux HTTP API, so Wasabi's `KruxBackend`/`KruxKeyChain` drive it unchanged; [`sabi-wallet.py`](kruxd/sabi-wallet.py) writes the wallet file (`CoinJoinVendor: 3`) HWI cannot. Untested against a live round yet | ❌ none. HWI already enumerates `BitBox02_BTCOnly`/`BitBox02_Multi` so ordinary PSBT signing works, but no `CoinJoinVendor`, no `VendorOf` entry, no `IKeyChain` |
